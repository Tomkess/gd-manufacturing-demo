...
- GoodData Cloud declarative artifacts under `analytics/`
- A small loader that refreshes a MotherDuck table from a public CSV (`data_pipeline/main.py`)
- A manual GitHub Actions workflow to run that refresh (`.github/workflows/production_line_pipeline.yml`)

## Dataset

The sample data is published as a public CSV:

- Source: `https://raw.githubusercontent.com/kubow/Data_playground/main/output/production_line.csv`
- Target table (default): `main.production_line`

## Quickstart

### 1) Load data into MotherDuck

Pick one of the following options.

#### Option A: Run the GitHub Actions pipeline (recommended)
...
3. Trigger the workflow: `Production line data pipeline`

You can override inputs at dispatch time:

- Target schema (defaults to `main`)
- Target MotherDuck database (defaults to the environment secret)
- Target table name (defaults to `production_line`)

Branch ↔ environment mapping is enforced by the workflow:
...
Create a local environment with **uv**, then install the loader dependency.
...
Run the loader (defaults to `main.production_line` and the public CSV URL):
...
```shell
uv run python -m data_pipeline.main --database "production_line" --schema "main" --table "production_line"
```

#### Option C: Create the table manually in MotherDuck

If you prefer a pure SQL approach in DuckDB/MotherDuck:

```sql
CREATE SCHEMA IF NOT EXISTS main;
CREATE OR REPLACE TABLE main.production_line AS
SELECT *
FROM read_csv_auto(
...
```shell
uv run python -c "from utils.motherduck import get_motherduck_token_b64; print(get_motherduck_token_b64())"
```

### 2) Deploy to GoodData Cloud

This project uses `gooddata.yaml` profiles (`dev`, `test`, `prod`) and reads credentials from environment variables:

- `GOODDATA_HOST`
- `GOODDATA_TOKEN`
- `GOODDATA_DATA_SOURCE`
- `GOODDATA_WORKSPACE_ID__DEV`
- `GOODDATA_WORKSPACE_ID__TEST`
- `GOODDATA_WORKSPACE_ID__PROD`

Then deploy the declarative artifacts from `analytics/`:
...
The sections below are not required to run the demo, but were kept as reference notes.

<details>
<summary>Free datasets available</summary>
...
<details>
<summary>Example dataset fields (legacy notes)</summary>
